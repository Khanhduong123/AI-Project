    ...
    def forward(self, dataloader,fw_model = 'train'):
        ...
        for i, data in enumerate(dataloader, 1):
            ...
            with torch.set_grad_enabled(fw_model == 'train'):
                ...
                preds = outputs.round()
                acc = self.compute_matrix(preds.softmax(dim=-1).argmax(dim=-1).tolist(), labels.tolist())
        ...
        for epoch in range(1, epochs+1):
            ...
            if val_loader is not None:
                try:
                    ...
                    val_acc = [str(k) + ": " + str(v) for k, v in self.cache['val_acc'][-1].items()]
                    val_acc = " - ".join(val_acc)
                    logs.append(f"\t=> Val epoch: loss: {val_loss} - {val_acc}")
